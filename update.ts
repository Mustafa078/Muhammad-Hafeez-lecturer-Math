export const newMathParticleSystem = `
function ParallaxGrid() {
  ...
  return (
    <motion.div
      style={{ y }}
      className="fixed inset-[-100%] z-[49] pointer-events-none opacity-20 math-grid"
      aria-hidden="true"
    />
  );
}
...
function MathParticleSystem() {
  ...
  return <canvas ref={canvasRef} className="fixed inset-0 z-[50] pointer-events-none mix-blend-plus-lighter opacity-60" aria-hidden="true" />;
}
`;
